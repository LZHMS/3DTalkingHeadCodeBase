...
def download_with_ytdlp(
    ...
    url2cate = defaultdict(str)
    for (url, start, end, category, duration) in iter_segments_from_big_json(input_json_path):
        url2cate[url] = category
    ...
    with ProcessPoolExecutor(max_workers=max(1, workers)) as executor:
        ...
        for url, segs in url2segments.items():
            ...
            category_dir = os.path.join(output_dir, url2cate[url])
            safe_mkdir(category_dir)
            future = executor.submit(
                run_yt_dlp_multi_sections,
                url, segs, category_dir, cookies_path, browser, extractor_args, True
            )
